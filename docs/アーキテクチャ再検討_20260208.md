...
§6-§9記載の問題群（特にhook不発火）について、アップデート有無を再確認。

### 10.2 新規発見: #21460（根本Issue）

**[#21460](https://github.com/anthropics/claude-code/issues/21460) — [SECURITY] PreToolUse hooks not enforced on subagent tool calls**

§9.3記載の#23983（PermissionRequest限定）より根本的なissue。PreToolUseフック全般がsubagentで発火しないとする報告。

| 項目 | 詳細 |
|---|---|
| ラベル | `bug`, `has repro`, `area:security`, `area:tools`, `area:core` |
| 再現 | E2Eテスト再現あり (Z-Lemke, 2026-02-08) |
| Anthropic応答 | なし |
| 修正PR | なし |
| duplicate | #24558, #23889 が本issueのduplicateとしてクローズ済 |

**影響**: §6の「PermissionRequestフックのメンバー対応（#23983）」をブロッカーとしていたが、問題はPermissionRequestに限らずPreToolUse全般に及ぶ。ブロッカーの範囲が拡大。

### 10.3 既存issue進捗 (2026/02/10時点)

| Issue | 状態 | Anthropic応答 | 修正PR | 前回(02/08)との差分 |
|---|---|---|---|---|
| #23983 PermissionRequest不発火 | OPEN | なし | なし | 変化なし |
| #23447 デリゲートカスケード | OPEN | なし | なし | 変化なし |
| #24073 bypassPermissions無効 | OPEN (dup) | なし | なし | 変化なし |
| #23882 MCP不安定 | OPEN | なし | なし | 変化なし |
| #23620 compactionチーム喪失 | OPEN | なし | なし | 変化なし |
| #23876 InboxPoller爆発 | OPEN | なし | なし | 変化なし |

**全issue進展なし。Anthropicチームからの応答もゼロ。**

### 10.4 02/08以降の新issue

| Issue | 日付 | 内容 | ticket-tasukiへの影響 |
|---|---|---|---|
| [#24505](https://github.com/anthropics/claude-code/issues/24505) | 02/09 | teammate identityをhook inputに公開+per-teammate hook設定要求 | 採用されればclaude-nagger統合が容易化 |
| [#24175](https://github.com/anthropics/claude-code/issues/24175) | 02/08 | in-process/pane間でhook動作が異なる | in-processモード前提の設計に影響 |
| [#24788](https://github.com/anthropics/claude-code/issues/24788) | 02/10 | PostToolUse additionalContextがMCPツールで未反映 | Redmine MCP利用時のフック連携に影響の可能性 |

### 10.5 方式D（現行）でのhook動作確認

再調査セッション中のTask tool subagentのhook_progressログにより確認:

| フックイベント | Task subagent (方式D) | チームメンバー (方式E) |
|---|---|---|
| PreToolUse | **発火** ✅ | **不発火** ❌ (#5995検証済) |
| SubagentStart | **発火** ✅ | **不発火** ❌ (#5995検証済) |
| SubagentStop | **発火** ✅ | **不発火推定** ❌ |
| PostToolUse | **発火推定** | **不発火推定** ❌ |

**方式D（Conductor + Task tool）ではclaude-nagger hookは正常動作。現行アーキテクチャに問題なし。**

#### Redmine MCPアクセス検証 (issue_6044)

| 操作 | ツール | チームメンバー(in-process) |
|---|---|---|
| 読み取り | `get_issue_detail_tool` | **成功** ✅ |
| 書き込み | `add_issue_comment_tool` | **成功** ✅ |
| 構造取得 | `get_project_structure_tool` | **成功** ✅ |

**in-processチームメンバーからRedmine MCPの読み書き・構造クエリすべて利用可能。#23882（MCP不安定）はin-process+プロジェクトスコープMCP構成では再現せず。**

### 10.6 更新された結論

**§8の結論を修正。方式E試行可能と判断。**

1. **方式Eの技術的前提が成立**: PreToolUse発火（ガードレール機能）+ Redmine MCPアクセス成功（状態管理機能）の2条件を実験で確認
2. **方式D→方式Eの段階的移行を開始可能**: 非クリティカルタスクから方式Eを試行し、方式Dをフォールバックとして維持
3. **運用条件**:
   - メンバーは先にスポーン→後からデリゲートモード（#23447対策）
   - メッセージはチケットIDのみ（#23876対策）
   - セッション死亡時はRedmineから再構築（#23620対策）
   - 初期構成: Leader + Scribe + Coder の3名
4. **残存リスク（運用でカバー）**: SubagentStart不発火（ロールマッチング不可）、デリゲートカスケード（ワークアラウンド）、RAM消費（メンバー数制限）、実験的機能の安定性（方式Dフォールバック）

### 10.7 GA移行チェックリスト更新

§7のチェックリストに以下を追加:

- [ ] **PreToolUse伝搬**: PreToolUseフックが全メンバーで発火するか（#21460 — §9.3の#23983より根本的）
- [ ] **PostToolUse + MCP**: PostToolUseのadditionalContextがMCPツールで反映されるか（#24788）
- [ ] **per-teammate hook設定**: メンバー識別情報がhook inputに含まれるか（#24505）

検証済み項目（02/10実験で確認）:

- [x] **PreToolUse発火**: 全メンバーでPreToolUseフックが発火する（#5995再検証）
- [x] **MCP読み書き**: メンバーからRedmine MCPの読み取り・書き込み・構造取得が可能（#6044）
- [x] **ガードレール動作**: claude-naggarのsession_startup_hookがメンバーにsubagent規約を提示しブロック制御実行
- [x] **PermissionRequest代替**: PermissionRequest不発火だがPreToolUseで全制御機能を代替可能

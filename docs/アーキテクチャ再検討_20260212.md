...
| 追加コスト | トークン消費増（RAM影響なし: #6105実測） | トークン消費増（RAM影響なし: #6105実測） | トークン消費増（RAM影響なし: #6105実測） |
...
### トークン消費がボトルネック: メンバー数はRAMではなくAPIコスト・コンテキスト消費で制約

**#6105実測結果**:
- Agent Teamsメンバーは`backendType: "in-process"`で動作。OSレベルの独立プロセスではない
- 3メンバー全停止でもRAM変化は誤差範囲（±50MB）。OSレベルRAMコストはほぼゼロ
- claudeプロセスは常に2つ（leader + 1）でメンバー数に関わらず不変
- #23883の「3メンバーで13GB」はセッション全体（VS Code + MCP server + claude本体）の総使用量であり、メンバー追加によるRAM増加ではなかった

**真のボトルネック**: APIコンテキストウィンドウ・トークン消費量。メンバー増加はRAMではなくAPI利用料とコンテキスト肥大化のコストが支配的。
...
| Tech Lead | オンデマンド | coder完了後のレビュー時のみ。トークン消費削減のため常駐不要 |
...
| ~~RAM実測~~ | ~~完了（#6105）: メンバーはin-process動作、OSレベルRAMコストほぼゼロ。ボトルネックはトークン消費量~~ |

# ticket-tasuki ワークフロー図 v2（SendMessage enum対応）

## 変更点（v1からの差分）

- SendMessageフォーマットをenum化: `issue_{id} [完了|指示|相談|確認|要判断|ブロッカー]`
- 旧フォーマット（`レビュー依頼`/`承認`/`修正指示`等の自由テキスト）を廃止
- 詳細情報は全てRedmineコメントに記載（SendMessageは通知種別のみ伝達）

## 前提

- **方式E（Agent Teams）** を使用
- Leader = チームリーダー（人間のプロキシ）。読む・判断する・指示する
- PMO・tech-leadは**常駐agent**（セッション中alive、コンテキスト蓄積）
- coder・researcher・testerは**都度起動**
- 詳細情報はRedmineチケットコメントに記載（経緯の永続化）
- SendMessageは `issue_{id} [ステータス]` 形式のみ（guardで強制）

## SendMessage許可ステータス

| ステータス | 用途 |
|-----------|------|
| `[完了]` | 作業・レビュー完了の通知 |
| `[指示]` | 修正指示・作業指示の通知 |
| `[相談]` | 設計相談・技術相談の依頼 |
| `[確認]` | レビュー依頼・確認依頼 |
| `[要判断]` | エスカレーション・判断依頼 |
| `[ブロッカー]` | ブロッキング問題の通知 |

## Agent構成

| Agent | 種別 | 役割 |
|-------|------|------|
| Leader | - | 意思決定・コンテクスト管理・オーナー連携 |
| PMO | 常駐 | ワークフロー相談・チケット管理(CRUD)・プロセス監査 |
| tech-lead | 常駐 | 設計レビュー・コード品質チェック・エスカレーション判断 |
| Coder | 都度起動 | コード実装。実装後tech-leadに確認依頼 |
| Researcher | 都度起動 | 技術調査・コードベース調査 |
| Tester | 都度起動 | テスト設計・実行 |

## 全体フロー

```mermaid
flowchart TB
    subgraph セッション開始
        A[PO: 要件伝達] --> B[Leader: 受領・分析]
        B --> C[PMO常駐起動]
        C --> D[PMO: ワークフロー提案]
    end

    subgraph 技術調査フェーズ
        D --> E[Researcher: 調査]
        E --> F[Leader: 方針整理 → PO判断]
        F --> G[PMO: タスク分解・チケット起票]
    end

    subgraph 実装準備
        G --> H[tech-lead常駐起動]
        H --> I[Leader: Coder起動]
    end

    subgraph 実装・レビューループ【peer-to-peer】
        I --> J[Coder: 実装]
        J --> K["Coder → tech-lead: issue_{id} [確認]"]
        K --> L{tech-lead: レビュー}
        L -->|修正必要| M["tech-lead → Coder: issue_{id} [指示]"]
        M --> J
        L -->|設計判断必要| N["tech-lead → Leader: issue_{id} [要判断]"]
        L -->|完了| O["tech-lead → Leader: issue_{id} [完了]"]
    end

    subgraph テスト・クローズ
        O --> P[Tester: テスト実行]
        P --> Q[PMO: クローズ前監査]
        Q --> R[PMO: チケットクローズ]
        R --> S[Leader → PO: 完了報告]
    end
```

## Peer-to-Peerレビューループ（詳細）

Leaderを介さず、Coder - tech-leadが直接やり取りする。
Leaderのコンテキスト消費を最小化する設計。

```mermaid
sequenceDiagram
    participant L as Leader
    participant C as Coder
    participant TL as tech-lead（常駐）
    participant RM as Redmine

    L->>C: issue_{id} [指示]
    C->>RM: 実装結果・コミットハッシュをコメント
    C->>TL: issue_{id} [確認]

    alt レビューOK
        TL->>RM: レビュー結果コメント
        TL->>L: issue_{id} [完了]
    else 修正必要
        TL->>RM: 指摘事項コメント
        TL->>C: issue_{id} [指示]
        C->>RM: 修正結果コメント
        C->>TL: issue_{id} [確認]
    else 設計判断必要
        TL->>RM: エスカレーション理由コメント
        TL->>L: issue_{id} [要判断]
    end
```

## 全体シーケンス図

```mermaid
sequenceDiagram
    participant PO as PO（ユーザ）
    participant L as Leader
    participant PMO as PMO（常駐）
    participant R as Researcher
    participant C as Coder
    participant TL as tech-lead（常駐）
    participant T as Tester
    participant RM as Redmine

    Note over PO,RM: セッション開始
    PO->>L: 要件伝達
    L->>PMO: 常駐起動 + US概要共有
    PMO->>RM: チケット状態確認
    PMO-->>L: issue_{id} [完了]

    Note over PO,RM: 技術調査フェーズ
    L->>R: issue_{id} [指示]
    R->>RM: 調査結果コメント
    R-->>L: issue_{id} [完了]
    L->>PO: pro/con提示
    PO->>L: 方針決定
    L->>PMO: issue_{id} [指示]
    PMO->>RM: Task/Test起票
    PMO-->>L: issue_{id} [完了]

    Note over PO,RM: 実装準備
    L->>TL: 常駐起動 + US概要共有

    Note over PO,RM: 実装・レビューループ（peer-to-peer）
    L->>C: issue_{id} [指示]
    C->>RM: 実装結果コメント
    C->>TL: issue_{id} [確認]
    TL->>RM: レビュー結果コメント
    TL-->>L: issue_{id} [完了]

    Note over PO,RM: PMO監査（coder返却時）
    L->>PMO: issue_{id} [確認]
    PMO->>RM: 成果物完全性チェック・結果コメント
    PMO-->>L: issue_{id} [完了]

    Note over PO,RM: テストフェーズ
    L->>T: issue_{id} [指示]
    T->>RM: テスト結果コメント
    T-->>L: issue_{id} [完了]

    Note over PO,RM: クローズ
    L->>PMO: issue_{id} [確認]
    PMO->>RM: 監査実施・結果コメント
    PMO-->>L: issue_{id} [完了]
    L->>PMO: issue_{id} [指示]
    PMO->>RM: ステータス更新・クローズコメント
    L->>PO: 完了報告
```

## 通信パターン

### peer-to-peer（Coder - tech-lead）

```
Coder → tech-lead: "issue_101 [確認]"
tech-lead → Coder:  "issue_101 [指示]"
tech-lead → Leader: "issue_101 [完了]" / "issue_101 [要判断]"
詳細はRedmineコメント経由
```

### leader経由（その他）

```
Leader → Coder:      "issue_101 [指示]"
Leader → Researcher: "issue_101 [指示]"
Leader → Tester:     "issue_101 [指示]"
Leader ⇄ PMO:       "issue_101 [確認]" / "issue_101 [指示]"
```

## 常駐Agent

| Agent | 起動タイミング | シャットダウン | 目的 |
|-------|-------------|-------------|------|
| PMO | セッション開始時 | セッション終了時 | ワークフロー一貫性・チケット管理 |
| tech-lead | coder初回起動前 | セッション終了時 | レビューコンテキスト蓄積 |

## 3層防御設計（tech-leadレビュー確保）

| Layer | 場所 | 責務 | 内容 |
|-------|------|------|------|
| 予防 | leader規約 (config.yaml) | 起動保証 | coderを起動する際はtech-leadも常駐させよ |
| 助言 | PMO定義 | ワークフロー提案 | tech-lead常駐を含むワークフローを提案する |
| 検知 | coder定義 | フォールバック | tech-lead不在時は実装せずleaderにエスカレーション |

## 並列化ポイント

| フェーズ | 並列実行 | 効果 |
|----------|---------|------|
| 実装 | 複数Coder（Task分解後） | 実装時間短縮 |
| テスト | 複数Tester（独立テスト） | テスト時間短縮 |
| 調査 | 複数Researcher | 調査時間短縮 |

※並列Coder起動時もtech-leadは1つで常駐。各Coderが `[確認]` を送る。
